...
st.set_page_config(page_title="QuantumDocs - Structured UI+", layout="wide")
st.title("QuantumDocs")
st.subheader("Unlock handwriting. Unleash insights.")
...
if uploaded_file:
    ...
    if st.button("Run Handwriting (EasyOCR) Mode"):
        if "pdf" not in file_type:
            handwriting_text = reader.readtext(np.array(image), detail=0)
            st.text_area("EasyOCR Result (Handwriting)", "\n".join(handwriting_text), height=150)
        else:
            st.warning("Handwriting mode only works on image files for now.")
else:
